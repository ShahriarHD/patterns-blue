...
export default function(boomApi: BoomApi) {
    ...
    const onHoverShape: TLPointerEventHandler = info => {
        boomApi.send('HOVERED_SHAPE', info);
    };

    const onUnhoverShape: TLPointerEventHandler = info => {
    ...
    const onPointCanvas: TLPointerEventHandler = info => {
        boomApi.send('POINTED_CANVAS', info);
    };

    const onPointBounds: TLPointerEventHandler = info => {
        boomApi.send('POINTED_BOUNDS', info);
    };

    const onPointHandle: TLPointerEventHandler = info => {
        boomApi.send('POINTED_HANDLE', info);
    };

    const onPointerDown: TLPointerEventHandler = info => {
        boomApi.send('STARTED_POINTING', info);
    };

    const onPointerUp: TLPointerEventHandler = info => {
    ...
    const onPinchStart: TLPinchEventHandler = info => {
    ...
    const onPinchEnd: TLPinchEventHandler = info => {
        boomApi.send('STOPPED_PINCHING', info);
    };

    const onPointBoundsHandle: TLPinchEventHandler = info => {
    ...
    const onKeyDown: TLKeyboardEventHandler = (key, info, event) => {
        switch (key) {
            case 'Alt':
            case 'Meta':
            case 'Control':
