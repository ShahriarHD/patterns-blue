...
import { ArrowShape, getShapeUtils } from '~/components/infinite-canvas/shapes';
...
export const rotateSelectedShapes: Action = (data, payload: TLPointerInfo) => {
    ...
    const initialCommonCenter = Utils.getBoundsCenter(Utils.getCommonBounds(selectedIds
    ...
    selectedIds.forEach(id => {
        ...
        if (shape.handles) {
            ...
            Object.values(shape.handles).forEach(handle => {
                invariant(initialShape.handles !== undefined, 'We should have some handles on our initialShapes');
