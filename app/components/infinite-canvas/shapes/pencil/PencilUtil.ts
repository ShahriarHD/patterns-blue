...
export class PencilUtil extends CustomShapeUtil<T, E> {
    ...
    getBounds = (shape: T) => {
        const bounds = Utils.getFromCache(this.boundsCache, shape, () => Utils.getBoundsFromPoints(shape.points));
    ...
    shouldRender = (prev: T, next: T): boolean => prev.points !== next.points;
