...
import { ArrowShape, Shape, getShapeUtils } from '~/components/infinite-canvas/shapes';
...
export const translateHandle: Action = (data, payload: TLPointerInfo) => {
    ...
    data.pageState.selectedIds.forEach(id => {
        ...
        const oppositeBindingTargetId = Object
            .values(data.page.bindings)
            .find(binding => binding.fromId === shape.id && binding.handleId === oppositeHandleId)?.toId;
        ...
        const oldBinding = Object
            .values(data.page.bindings)
            .find(binding => binding.fromId === shape.id && binding.handleId === pointedHandleId);
