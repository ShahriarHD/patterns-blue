...
export const extendPencilShape: Action = data => {
    const { initialPoint, previousPoint, rawPoints, pointedShapeId } = mutables;

    if (Vec.isEqual(previousPoint, mutables.currentPoint)) {
        return;
    }

    invariant(pointedShapeId !== undefined, 'We should have a pointed shape id');
